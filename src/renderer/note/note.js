...
const addBtn = document.getElementById('add-btn');
 ...
 // Tự động đồng bộ Cloud ẩn (Logic nằm ở Main Process)
ipcRenderer.on('cloud-data-synced', (e, data) => {
    if (data) {
        localStorage.setItem('rpg-system-v2', JSON.stringify(data));
        // Reload nhẹ UI hoặc cập nhật biến
        location.reload(); 
    }
});
...
async function loadGoogleTasks() {
    taskListEl.innerHTML = '<div style="text-align:center; opacity:0.6; padding:10px; font-weight:bold; color: #d84315;">Cắm ống API Google... ☁️</div>';
    try {
...
function renderTasks() {
    ...
    if (tasks.length === 0) {
        taskListEl.innerHTML = '<div style="text-align:center; opacity:0.5; font-size: 14px; margin-top:20px;">Sổ tay trống rỗng! Hãy lên kế hoạch.</div>';
        return;
    }
...
async function addTask() {
    ...
    // Optimistic Update: Thêm Nhiệm Vụ Ảo với ID tạm thời duy nhất
    const tempId = 'temp-' + Date.now();
    tasks.push({ id: tempId, text: v + ' [Đang Sync] ☁️', done: false, loading: true });
    renderTasks();
    ...
    try {
        ...
        if (gTask) {
            // Tìm và cập nhật đúng task dựa trên tempId
            const taskIndex = tasks.findIndex(t => t.id === tempId);
            if (taskIndex !== -1) {
                tasks[taskIndex].id = gTask.id;
                tasks[taskIndex].text = v;
                delete tasks[taskIndex].loading;
                renderTasks();
            }
        }
    } catch (err) {
        console.error("Lỗi thêm task:", err);
        // Xoá task tạm nếu lỗi
        const taskIndex = tasks.findIndex(t => t.id === tempId);
